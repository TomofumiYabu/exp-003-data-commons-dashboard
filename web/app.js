...
function chart(d,v){
 ...
 svg+='</svg>';
 return `<article class="chart-card"><h3>${v.label}</h3><p class="chart-meta">${v.key==='population'?'Millions of people':v.unit} · ${Math.min(...points.map(p=>p.year))}–${Math.max(...points.map(p=>p.year))}</p><div class="chart-viewport" tabindex="0" aria-label="Scrollable ${v.label} chart">${svg}</div><p class="mobile-chart-help">Swipe or scroll horizontally to read all years.</p><p class="chart-note">World Bank WDI via Data Commons MCP. ${v.key==='life'?'Vertical scale begins at 50 years. ':''}${v.key==='population'?'2024 available.':'2024 missing; no extrapolation.'}</p></article>`;
}
